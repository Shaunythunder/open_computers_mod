...
/* 23 */     if (environmentHost instanceof li.cil.oc.api.internal.Adapter) { EnvironmentHost environmentHost1 = environmentHost; managedEnvironment = (ManagedEnvironment)new UpgradeTankController.Adapter(environmentHost1); }
/* 24 */     else if (environmentHost instanceof Drone) { Drone drone = (Drone)environmentHost; managedEnvironment = (ManagedEnvironment)new UpgradeTankController.Drone((Agent)drone); }
/* 25 */     else if (environmentHost instanceof Robot) { Robot robot = (Robot)environmentHost; managedEnvironment = (ManagedEnvironment)new UpgradeTankController.Robot(robot); }
